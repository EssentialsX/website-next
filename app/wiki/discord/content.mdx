...
EssentialsX Discord offers _essential_ features you'd want from a Discord bridge such as:
...
> - [Initial Setup](#initial-setup)_ [Console Relay](#console-relay)_ [Configuring Messages](#configuring-messages)_ [Receive Discord Messages in Minecraft](#receive-discord-messages-in-minecraft)_ [Discord Commands](#discord-commands)_ [Misc Permissions](#misc-permissions)_ [Developer API](#developer-api)
...
> ![Creating Application](./images/discord/create-app.gif) > `New Application` -> Give Application a Name -> `Create`
...
> ![Copy Application ID](./images/discord/copy-client-id.gif) > `Copy` -> Paste into Notepad for later step
...
> ![Create Bot](./images/discord/create-bot.gif) > `Bot` -> `Add Bot` -> `Yes, do it!`
...
> ![Copy Token](./images/discord/copy-token.gif) > `Reset Token` -> `Yes, do it!` -> `Copy` -> Paste into Notepad for later step
...
> ![Developer Mode](./images/discord/dev-mode.gif) > `User Settings` -> `Advanced` -> Check `Developer Mode` -> Exit Settings
...
13. You've successfully copied all the necessary IDs needed for a basic setup. Next up is generating the
    ...
    EssentialsX Discord jar (you can download it [here](https://essentialsx.net/downloads) if you do not
...
> ![/execute](./images/discord/execute-command.gif)
...
numbers or dashes. You _can_ also put a Discord channel ID as the key if you'd like to
