...
import { getCommitIdFromArtifact, getModuleIdFromArtifact, getVersionFromArtifact } from "@/lib/build-utils";
...
export const SharedDataProvider = ({children}: { children: React.ReactNode }) => {
    ...
    const [stableBuild, setStableBuild] = useState<BuildData | undefined>(undefined);
    ...
    const fetchGithubData = async () => {
        ...
        let forks = 1000;
        try {
            const response = await axios.get("https://api.github.com/repos/EssentialsX/Essentials");
    ...
    const fetchDiscordData = async () => {
        let members = 1000;
        try {
            const response = await axios.get("https://canary.discordapp.com/api/guilds/390942438061113344/widget.json");
    ...
    const fetchBuildData = async () => {
        ...
        try {
            ...
            data[0].assets.forEach((asset: { name: string, browser_download_url: string }) => {
        ...
        try {
            ...
            version = getVersionFromArtifact(response.data.artifacts[0].displayPath) ?? 'Unknown';
            if (response.data.changeSet.items[0]) {
                commit = response.data.changeSet.items[0].commitId;
            } else {
                commit = getCommitIdFromArtifact(response.data.artifacts[0].displayPath) ?? 'Unknown'
            }

            response.data.artifacts.forEach((artifact: { displayPath: string, relativePath: string }) => {
                const moduleId = getModuleIdFromArtifact(artifact.displayPath);
                downloads[moduleId] = `${currentCI}lastSuccessfulBuild/artifact/${artifact.relativePath}`;
            });
    ...
    return (
        <SharedDataContext.Provider value={{github, patreon, discord, devBuild, stableBuild, downloads}}>
            {children}
